...
[![builds.sr.ht status](https://builds.sr.ht/~jcmuller/figleaf/commits/main.svg)](https://builds.sr.ht/~jcmuller/figleaf/commits/main?)
...
`Figleaf::Settings` can be used to override settings depending on what environment your application is running.
If it's a Rails app, it will know it from `Rails.env`, otherwise it will check for `ENV['ENVIRONMENT']`.
...
Also, it provides the ability for you to define all your environment dependent settings in just one YAML file inside `config/settings/`.
The anatomy of these files should be:
